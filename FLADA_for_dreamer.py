...
# 在三个中随机选一个
subject_files = files
index_chose = torch.randperm(len(subject_files))[0]
target_file = subject_files[index_chose]
...
class Classifier(BasicModule):
    def __init__(self):
        ...
        self.seq = nn.Sequential(nn.Linear(16,5),
                                 nn.Softmax(dim=1))
    def forward(self,x):
        out = self.seq(x)
        return out
...
maximum = -1
train_dataloader = data_loader_train(source_file)  # loading the dataset source and target
test_dataloader = data_loader_test(target_file)
...
#     classifier = Classifier(64,128,2,3)
#     encoder = Encoder()
#     discriminator = DCD(in_features=128)
